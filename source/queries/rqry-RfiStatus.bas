dbMemo "SQL" ="SELECT tblRFI.RfiID, IIf([rfiCanceled]=True,\"CANCELED\",IIf([rfiSatisfied]=True"
    ...
    "FROM ((((tblRFI INNER JOIN qdApplicant ON (tblRFI.DisasterID = qdApplicant.Disas"
    "terID) AND (tblRFI.ApplicantID = qdApplicant.ApplicantID)) LEFT JOIN [rqry-Disti"
    ...
    "der) LEFT JOIN qryRfiReviewCompletionTransform ON (tblRFI.RfiID = qryRfiReviewCo"
    "mpletionTransform.RfiID) AND (tblRFI.DisasterID = qryRfiReviewCompletionTransfor"
    "m.DisasterID) AND (tblRFI.ApplicantID = qryRfiReviewCompletionTransform.Applican"
    "tID)) LEFT JOIN [rqry-RfiCountOfOpenItems] ON tblRFI.RfiID = [rqry-RfiCountOfOpe"
...
dbMemo "Filter" ="((([rqry-RfiStatus].[RfiStatus]=\"IN PROGRESS\"))) AND ([rqry-RfiStatus].[RFI Cr"
    "eation]<>\"Pending\" Or [rqry-RfiStatus].[RFI Creation] IS Null)"
